...
elastcv_forecast = function(Y_or, Y, X, lambdas, h, ratio_start = 0.8, alpha = 0.5) {
  n_tot <- length(Y)
  ...
  for(i in 1:n_out){
    ...
    for(j in 1:h) {
      ...
      # finding m*
      model_cv <- cv.glmnet(x_reg, y_reg,
                             alpha = alpha, lambda = lambdas,
                             standardize = TRUE, nfolds = 10)
      # Best 
      optimal_lambda <- model_cv$lambda.min
      model <- glmnet(x_reg, y_reg, alpha = alpha, lambda = optimal_lambda,
                            family = 'gaussian',
                            standardize = TRUE)
      predictions_test <- predict(model, s = optimal_lambda,
                                  newx = x0_reg)
