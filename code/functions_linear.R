#############################################################
## Linear functions for simulations
##
## gslindenmeyer@gmail.com - Oct 2021
############################################################


## takes a df, h, p and number of observations left for training to create a linear model
linear_model <- function(yt, xt, h, p, n_out = 200) {
  ## yt - time series of interest - it must be stationary
  # h - horizon forecast
  # p - usually between 1-12. How many lags to include in the predictor vector

  df <- data.frame(
    lag_1 = NA, lag_2 = NA, lag_3 = NA, lag_4 = NA, lag_5 = NA, lag_6 = NA, # here we create an empty data frame to work with
    lag_7 = NA, lag_8 = NA, lag_9 = NA, lag_10 = NA, lag_11 = NA, lag_12 = NA
  )
  df <- df[, 1:p] # then we cut it depending of your choosed p value

  for (i in 1:p) { # creation of data frame for the predictos. We want to use for instance
    # df as yt+1 = yt + e
    #  cat((i/p)*100, '%\n')
    if (p > 1) {
  ...
  for (i in 1:n_out) {
    ...
    if (p > 1) {
      ...
      x0_reg <- data.frame(lag_1 = data[(ind_out[i] - h + 1), colnames(df)]) # this is the value that we will use to predict it + h
    ...
    for (j in 1:1) {
      ...
      data2 <- as.data.frame(cbind(y_reg = y_dep, x_reg)) # here we are building
      # the data to create the function y_reg = lag_1 + e
      ...
      bic_predicted <- c(bic_predicted, BIC(model_1))
...
## Run linear model several times and discovers the best model
best_linear_model <- function(yt, xt, n_out_out) {
  ...
  for (j in 1:12) {
    ...
    for (i in 1:12) {
      ...
      df_linear_estimate[i, j] <- mean(model$bic)
...
catch_r2 <- function(yt, h_max = 12, p_out = 12, n_out_out = 200, name = NULL, best = F) {
  ...
  if (best) {
    ...
    linear_estimate <- catch_r2(yt,
      h_max = 12, p_out = best$p_linear_estimate,
      n_out_out = n_out_out, name = paste(name, "linear_estimate", sep = ""), best = F
    )
    best_linear <- catch_r2(yt,
      h_max = 12, p_out = best$p_best_linear,
      n_out_out = n_out_out, name = paste(name, "best_linear", sep = ""), best = F
    )
