...
data_test = readMat('FREDTests/FRED_MSW_recursive_stationary_1959_1999_216_1(RPI).mat') ### !!!! you need to fix this path
...
for(h in 1:12){
  ...
  ind =(1-mse(xt,tyt)/var(tyt[481:696]))# variance  ##     ################### descomentar esta linha e comentar a debaixo  para trocar
  # ind =(1-mse(xt,tyt)/mse(var,tyt)) # constant pred
  
  model = c(model,   ifelse(ind<0,0,ind) )
  ind =(1-mse(xt2,tyt)/ var(tyt[481:696]))# variance  ##     ################### descomentar esta linha e comentar a debaixo  para trocar
  # ind =(1-mse(xt2,tyt)/ mse(var,tyt)) #const pred
